...
One_Dimensional_AGP_D::One_Dimensional_AGP_D(double _a, double _b, double _curr_x,
	double(*f)(double x, double y), int _k_par, double _q, double _Q,
...
void One_Dimensional_AGP_D::compute_R(double new_point, double new_m) {
	while (!pq->empty())
		pq->pop();
	reset();
	for (; right_point != points->end(); go_Next_Interval()) {
...
void One_Dimensional_AGP_D::insert_to_map(double _y, double _z,
