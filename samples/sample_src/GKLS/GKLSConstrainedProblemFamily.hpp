...
{
public:
  TGKLSConstrainedProblemFamily(EConstrainedProblemType problemType = cptNormal,
    double fraction = 0.5, int activeConstrNum = 0, int dim = 2,
    GKLSClass type = Simple, GKLSFuncionType functionType = TND) : IConstrainedOptProblemFamily()
  {
    for (int i = 1; i <= 100; i++)
